...
var NotifierErrorHandler = /** @class */ (function (_super) {
    tslib__WEBPACK_IMPORTED_MODULE_0__["__extends"](NotifierErrorHandler, _super);
    function NotifierErrorHandler(notifierService) {
        var _this = _super.call(this) || this;
        _this.notifierService = notifierService;
        return _this;
    }
    NotifierErrorHandler.prototype.handleError = function (error) {
        this.notifierService.notify('error', error);
        console.error(error);
    };
    NotifierErrorHandler.ctorParameters = function () { return [
        { type: angular_notifier__WEBPACK_IMPORTED_MODULE_2__["NotifierService"] }
    ]; };
    return NotifierErrorHandler;
...
var DirectivesHelper = /** @class */ (function () {
    function DirectivesHelper() {
    }
    DirectivesHelper.toReplace = function (palette, omitReplace) {
        if (omitReplace === void 0) { omitReplace = false; }
        var s = omitReplace ? '' : '?replace';
        Object.keys(palette).forEach(function (key) {
            s += ";" + key + "=" + palette[key];
        });
        return s;
    };
    return DirectivesHelper;
}());
...
var FileHelper = /** @class */ (function () {
    ...
    FileHelper.readFileAsDataURL = function (file) {
        var subj = new rxjs__WEBPACK_IMPORTED_MODULE_0__["Subject"]();
    ...
    FileHelper.saveText = function (text, fileName) {
...
/* harmony import */ var upng_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! upng-js */ "./node_modules/upng-js/UPNG.js");
/* harmony import */ var upng_js__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(upng_js__WEBPACK_IMPORTED_MODULE_2__);
...
var ImageHelper = /** @class */ (function () {
    ...
    ImageHelper.fromBase64 = function (base64) {
        if (base64.substr(0, 4) === 'data') {
            base64 = base64.substr(22);
        }
        var arr = Uint8Array.from(atob(base64), function (c) { return c.charCodeAt(0); });
        return Object(upng_js__WEBPACK_IMPORTED_MODULE_2__["decode"])(arr);
    };
    ImageHelper.fromFile = function (file) {
        return _root_helpers_file_helper__WEBPACK_IMPORTED_MODULE_3__["FileHelper"].readFileAsDataURL(file).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_1__["map"])(function (data) { return ImageHelper.fromBase64(data); }));
    };
    ImageHelper.toColorTable = function (image, options) {
        options = options || {};
        var buffer = Object(upng_js__WEBPACK_IMPORTED_MODULE_2__["toRGBA8"])(image)[0];
        ...
        for (var y = 0; y < image.height; y++) {
            ...
            for (var x = 0; x < image.width; x++) {
                var pixel = [view[i++], view[i++], view[i++], view[i++]];
                if (options.skipTransparent && pixel[3] === 0) {
                    rows[y].push(undefined);
                    continue;
                }
                var hex = this.toHex(pixel);
                rows[y].push(hex);
            }
        }
        return rows;
     ...
     */
    ImageHelper.toHex = function (bytes) {
        var cp = tslib__WEBPACK_IMPORTED_MODULE_0__["__spread"](bytes);
        if (cp[3] === 255) {
            delete cp[3];
        }
        var shorten = cp.every(function (b) { return (b / 17) % 1 === 0; });
        var s = cp.map(function (n) { return ("0" + n.toString(16)).slice(shorten ? -1 : -2); }).join('');
        return s;
    };
    ImageHelper.difference = function (from, to) {
        var swaps = {};
        from.forEach(function (row, y) {
            row.forEach(function (a, x) {
                var b = to[y] && to[y][x];
                if (typeof b !== 'string') {
                    return;
                }
...
var TemplateHelper = /** @class */ (function () {
     ...
     * ]);
     */
    TemplateHelper.create = function (frameWidth, frameHeight, frames) {
        var verticalFrames = frames.length;
        var horizontalFrames = frames[0].length;
        var v = verticalFrames * frameHeight;
        // Build arrray
        var rows = [];
        for (var i = 0; i < v; i++) {
            rows.push([]);
        ...
        // For every frame
        for (var w = 0; w < horizontalFrames; w++) {
            for (var h = 0; h < verticalFrames; h++) {
                // Frame identifier
                var id = frames[verticalFrames - 1 - h][w];
                if (!id) {
                    continue;
                }
                ...
                for (var x = 0; x < frameWidth; x++) {
                    for (var y = 0; y < frameHeight; y++) {
                        var pixel = _image_helper__WEBPACK_IMPORTED_MODULE_0__["ImageHelper"].toHex([x, parseInt(id, 16), y, 0]);
...
var ImageInputComponent = /** @class */ (function () {
    ...
    ImageInputComponent.prototype.clearOld = function () {
        this.input.nativeElement.value = '';
    };
    ImageInputComponent.prototype.fileChange = function (event) {
        var _this = this;
        ...
        if (!files || files.length === 0) {
            ...
            _root_helpers_file_helper__WEBPACK_IMPORTED_MODULE_3__["FileHelper"].readFileAsDataURL(files[0]).subscribe({
                next: function (base64) {
                    try {
                        var image = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_2__["ImageHelper"].fromBase64(base64);
                        _this.imageLoaded.emit({ image: image, dataUrl: base64 });
                    }
                    catch (e) {
                        event.target.value = '';
                        _this.imageError.emit(e);
                    }
                },
                error: function (e) {
                    event.target.value = '';
...
var DashboardComponent = /** @class */ (function () {
    ...
    DashboardComponent.prototype.beforeChange = function (event) {
        console.log(event);
    };
...
var TabBackComponent = /** @class */ (function () {
    function TabBackComponent(notifierService) {
        this.notifierService = notifierService;
        this.fileTemplates = _back_file_templates__WEBPACK_IMPORTED_MODULE_2__["backFileTemplates"];
    }
    TabBackComponent.prototype.imageLoaded = function (imageModel) {
        this.imageModel = imageModel;
    };
    TabBackComponent.prototype.imageCleared = function () {
        this.imageModel = undefined;
    };
    TabBackComponent.prototype.imageError = function (error) {
        this.imageCleared();
        this.notifierService.notify('error', error);
    };
    TabBackComponent.prototype.generate = function () {
        var descriptor = JSON.parse(JSON.stringify(_back_descriptor__WEBPACK_IMPORTED_MODULE_3__["backDescriptor"]));
        var res = '?replace';
        if (this.imageModel) {
            var pants = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_4__["ImageHelper"].toColorTable(this.imageModel.image, { skipTransparent: true });
            var t = _root_helpers_template_helper__WEBPACK_IMPORTED_MODULE_5__["TemplateHelper"].create(43, 43, [
            ...
            var diff = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_4__["ImageHelper"].difference(t, pants);
            res += _root_helpers_directives_helper__WEBPACK_IMPORTED_MODULE_6__["DirectivesHelper"].toReplace(diff, true);
        }
        var full = _root_helpers_templates__WEBPACK_IMPORTED_MODULE_9__["templates"].back.join('');
    ...
    TabBackComponent.ctorParameters = function () { return [
...
var TabChestComponent = /** @class */ (function () {
    function TabChestComponent(notifierService) {
        this.notifierService = notifierService;
        this.images = {};
        this.fileTemplates = _chest_file_templates__WEBPACK_IMPORTED_MODULE_7__["chestFileTemplates"];
    }
    TabChestComponent.prototype.imageLoaded = function (imageModel, slot) {
        this.images[slot] = imageModel;
    };
    TabChestComponent.prototype.imageCleared = function (slot) {
        this.images[slot] = undefined;
    };
    TabChestComponent.prototype.imageError = function (error, slot) {
        this.imageCleared(slot);
        this.notifierService.notify('error', error);
    };
    TabChestComponent.prototype.generate = function () {
        var descriptor = JSON.parse(JSON.stringify(_chest_descriptor__WEBPACK_IMPORTED_MODULE_8__["chestDescriptor"]));
        var res = '?replace';
        if (this.images.chest) {
            var chest = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_3__["ImageHelper"].toColorTable(this.images.chest.image, { skipTransparent: true });
            var chestTemplate = _root_helpers_template_helper__WEBPACK_IMPORTED_MODULE_4__["TemplateHelper"].create(43, 43, [
                ['', 'f3'],
                ['f4', 'f5'],
                ['', 'f6'],
                ['', 'f7'],
            ...
            var diff = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_3__["ImageHelper"].difference(chestTemplate, chest);
        ...
        if (this.images.frontSleeves) {
            var front = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_3__["ImageHelper"].toColorTable(this.images.frontSleeves.image, { skipTransparent: true });
            var frontSleeveTemplate = _root_helpers_template_helper__WEBPACK_IMPORTED_MODULE_4__["TemplateHelper"].create(43, 43, [
                ['', 'a1', 'a2', 'a3', 'a4', 'a5', '', '', 'a6'],
                ['', '', 'b1', 'b2', 'b3', 'b4', 'b5', '', 'b6'],
                ['', '', 'c1', 'c2', 'c3', 'c4', 'c5', '', ''],
                ['', 'd1', 'd2', 'd3', 'd4', 'd5', 'd6', 'd7', 'd8'],
            ...
            var diff = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_3__["ImageHelper"].difference(frontSleeveTemplate, front);
            res += _root_helpers_directives_helper__WEBPACK_IMPORTED_MODULE_5__["DirectivesHelper"].toReplace(diff, true);
        }
        if (this.images.backSleeves) {
            var back = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_3__["ImageHelper"].toColorTable(this.images.backSleeves.image, { skipTransparent: true });
            var backSleeveTemplate = _root_helpers_template_helper__WEBPACK_IMPORTED_MODULE_4__["TemplateHelper"].create(43, 43, [
                ['', '60', '61', '62', '63', '64', '', '', '65'],
                ['', '', '66', '67', '68', '69', '6a', '', '6b'],
                ['', '', '6c', '6d', '6e', '6f', '70', '', ''],
            ...
            var diff = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_3__["ImageHelper"].difference(backSleeveTemplate, back);
            res += _root_helpers_directives_helper__WEBPACK_IMPORTED_MODULE_5__["DirectivesHelper"].toReplace(diff, true);
        }
        var full = _root_helpers_templates__WEBPACK_IMPORTED_MODULE_6__["templates"].chest.join('');
        descriptor.parameters.directives = "" + full + res;
    ...
    TabChestComponent.ctorParameters = function () { return [
        { type: angular_notifier__WEBPACK_IMPORTED_MODULE_2__["NotifierService"] }
    ]; };
...
var TabHatComponent = /** @class */ (function () {
    function TabHatComponent() {
    }
    TabHatComponent.prototype.ngOnInit = function () {
    };
...
var TabPantsComponent = /** @class */ (function () {
    function TabPantsComponent(notifierService) {
        this.notifierService = notifierService;
        this.fileTemplates = _pants_file_templates__WEBPACK_IMPORTED_MODULE_2__["pantsFileTemplates"];
    }
    TabPantsComponent.prototype.imageLoaded = function (imageModel) {
        this.imageModel = imageModel;
    };
    TabPantsComponent.prototype.imageCleared = function () {
        this.imageModel = undefined;
    };
    TabPantsComponent.prototype.imageError = function (error) {
        this.imageCleared();
        this.notifierService.notify('error', error);
    };
    TabPantsComponent.prototype.generateNormal = function () {
    ...
    TabPantsComponent.prototype.generateHiding = function () {
    ...
    TabPantsComponent.prototype.generate = function (descriptor, template) {
        var res = '?replace';
        if (this.imageModel) {
            var pants = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_5__["ImageHelper"].toColorTable(this.imageModel.image, { skipTransparent: true });
            var t = _root_helpers_template_helper__WEBPACK_IMPORTED_MODULE_6__["TemplateHelper"].create(43, 43, [
                ['', 'a1', 'a2', 'a3', 'a4', 'a5', 'a6', '', 'a7'],
                ['', 'b1', 'b2', 'b3', 'b4', 'b5', 'b6', 'b7', 'b8'],
                ['', 'c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7', 'c8'],
                ['', 'd1', 'd2', 'd3', 'd4', 'd5', 'd6', 'd7', 'd8'],
            ...
            var diff = _root_helpers_image_helper__WEBPACK_IMPORTED_MODULE_5__["ImageHelper"].difference(t, pants);
            res += _root_helpers_directives_helper__WEBPACK_IMPORTED_MODULE_8__["DirectivesHelper"].toReplace(diff, true);
        }
        var full = template.join('');
        descriptor.parameters.directives = "" + full + res;
    ...
    TabPantsComponent.ctorParameters = function () { return [
...
var FileTemplateContainerComponent = /** @class */ (function () {
    function FileTemplateContainerComponent() {
    }
    FileTemplateContainerComponent.prototype.ngOnInit = function () {
    };
    FileTemplateContainerComponent.prototype.fileName = function (path) {
    ...
    return FileTemplateContainerComponent;
}());
